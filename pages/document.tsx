...
export default function DocumentForm() {
  ...
  const isMontantInvalid = () =>
    isEmpty(form.montant) || Number(form.montant) <= 0;
  ...
  return (
    <div className="flex min-h-screen bg-blue-50 text-gray-900">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center mb-3">
                ...
                <div className="flex items-center gap-2 flex-1">
                  <input
                    className={`p-2 rounded w-40 border ${isMontantInvalid() ? "border-red-500" : "border-gray-300"}`}
                    name="montant"
                    value={form.montant}
                    onChange={handleChange}
                    type="number"
      ...
      >
        ...
        >
          <div style={{ position: "absolute", top: 57, right: 520 }}>
            {form.echeance}
          </div>
          <div style={{ position: "absolute", top: 61, right: 390 }}>
            {creationDate}
          </div>
          <div style={{ position: "absolute", top: 41, right: 390 }}>
            {form.lieu}
          </div>
          ...
          <div
            style={{ position: "absolute", top: 90, right: 220 }}
          >{`${form.montant}.${form.millimes.padStart(3, "0")} DT`}</div>

          <div style={{ position: "absolute", top: 150, left: 183 }}>
            {form.beneficiaire}
          </div>

          <div
            style={{ position: "absolute", top: 145, right: 220 }}
          >{`${form.montant}.${form.millimes.padStart(3, "0")} DT`}</div>
          ...
          <div className="absolute top-[210px] left-[114px]">
            {creationDate}
          </div>
          ...
          <div className="absolute top-[300px] left-[145px] w-[150px]">
            {form.aval}
          </div>
...
function convertirNombreEnLettres(n: number): string {
  ...
  for (const groupe of groupes) {
    ...
    if (groupe.valeur === 1e3) {
      ...
      mots.push(
        convertirNombreEnLettres(q) +
          " " +
          (q > 1 ? groupe.nomPluriel : groupe.nomSingulier),
      );
    }
  }
...
function convertirMontantEnLettres(dinars: number, millimes: number): string {
  const dinarsTxt =
    dinars > 0
      ? `${convertirNombreEnLettres(dinars)} dinar${dinars > 1 ? "s" : ""}`
      : "";
  const millimesTxt =
    millimes > 0
      ? `${convertirNombreEnLettres(millimes)} millime${millimes > 1 ? "s" : ""}`
      : "";
